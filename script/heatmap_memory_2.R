...
#corregir el plato b1, plato b2 y c2
a<-c()
b<-c()
for (j in 1:384) {
  for (i in 1:19){
    a[i]<-mean(plate_a_1[i,j],plate_b_1[i,j],plate_c_1[i,j])
    b<-cbind(b,a)
  }
}

plate_b_1<-matrix(b,nrow = 19,ncol = 384)
...
memory<-select(memory,-c(dcuS,torR))
non_memory<-select(non_memory,-c(dcuS,torR))
...
memory_plot<-ggplot(memory, aes(Time, variable,fill=value)) + 
  ...
  scale_fill_gradientn(colours = pal,name = "Estrés/Control")  +
  theme_classic() +
