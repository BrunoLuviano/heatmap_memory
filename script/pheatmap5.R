...
source(here("script","orden4.R"))

estres_2<-a
estres_1<-c
strains_2<-as.vector(t(strains_2))

network<-network[,strains_2] #datos reordenados

#la variable diff_exp es importarte, contiene la información  de log(Estres2/Estres1)
estres_2<-as.data.frame(t(estres_2))
estres_1<-as.data.frame(t(estres_1))
annotation_row<-estres_2
annotation_col<-estres_1
