...
#seleccionar SOLO los reguladores incluidos en el experimento
reguladores<-read.csv(here("data","lista_reguladores_genes_heatmap.csv"))
genes<-as.vector(reguladores[,2])
reguladores<-as.vector(reguladores[,1])

network<-as.data.frame(t(network)) #se saca transpuesta
network<-select(network,reguladores)

#la variable diff_exp es importarte, contiene la información  de log(Estres2/Estres1)
regulador<-diff_exp
#regulador<-select(regulador,-c(basR,rcsAB))
regulador<-select(regulador,genes)
diff_exp<-as.data.frame(t(diff_exp))
regulador<-as.data.frame(regulador)
colnames(regulador)<-reguladores
regulador<-as.data.frame(t(regulador))
annotation_row<-diff_exp #este vector incluye expresiones de genes
annotation_col<-regulador #expresion de reguladores
...
pheatmap::pheatmap(network, 
                   ...
                   color = c("snow2", "red3","blue3","green3","turquoise3"), 
                   fontsize = 6.5,
                   fontsize_row=2, 
                   fontsize_col = 3)
