...
a<-diff_exp
#seleccionar genes que se expresaron a niveles similares
n<-ncol(a)
number<-c(1:n)

a<-rbind(a,number)
a<-as.data.frame(t(a))
a$diff_exp<-abs(a$diff_exp)
b<-min(a$diff_exp)
a<-a[a$diff_exp==b,]
a<-a[,2]
...
network<-network[,c(1:m,c:b,l:n)]
diff_exp<-diff_exp[,c(1:m,c:b,l:n)]
...
n<-ncol(network)
#m es el mismo valor de antes
l<-n - m
network_1<-network[,c(1:m)]
network_2<-network[,-c(1:m,l:n)]
network_3<-network[,c(l:n)]

network_2[network_2 == 1] <- 2
network_3[network_3 == 1] <- 3

#unir las partes
network<-cbind(network_1,network_2,network_3)
...
pheatmap::pheatmap(network, 
                   ...
                   color = c("snow2", "red3","green3","blue3") , 
                   fontsize = 6.5,
                   fontsize_row=7, 
                   fontsize_col = 8)
