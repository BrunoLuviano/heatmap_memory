...
#eliminar
a<-select(a,-c(dcuS,U66))
c<-select(c,-c(dcuS,U66))
...
a_plot<-ggplot(a , aes(x = variable , y = value, fill = value)) +
  geom_bar(position = 'identity', stat = "identity") +
  scale_fill_viridis_c(name="Valor") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust = 0.5)) +
  scale_x_discrete("Genes",expand=c(0,0)) +
  scale_y_discrete("ln(Área bajo la curva(Estrés 2/Control))",expand=c(0,0)) +
  theme(axis.text = element_text(size = 2),
        title = element_text(size = 12,face="bold"),
        panel.border= element_rect(size=0.5,color="black",fill=NA)) +
  coord_flip()

c_plot<-ggplot(c , aes(x = variable , y = value, fill = value)) +
  geom_bar(position = 'identity', stat = "identity") +
  scale_fill_viridis_c(name="Valor") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust = 0.5)) +
  scale_x_discrete("Genes",expand=c(0,0)) +
  scale_y_discrete("ln(Área bajo la curva(Estrés 1/Control))",expand=c(0,0)) +
  theme(axis.text = element_text(size = 2),
        title = element_text(size = 12,face="bold"),
        panel.border= element_rect(size=0.5,color="black",fill=NA)) +
  coord_flip()
...
figure <- ggarrange(c_plot,a_plot,
