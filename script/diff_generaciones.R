...
#calcular la tasa de divisiones bacterianas
diff_a_1<-diff(a_1)
diff_a_2<-diff(a_2)
diff_a_3<-diff(a_3)
diff_a_4<-diff(a_4) 
...
#unir los datos
plate_a<-c(diff_a_1,diff_a_2,diff_a_3,diff_a_4,diff_a_5)
plate_b<-c(diff_b_1,diff_b_2,diff_b_3,diff_b_4,diff_b_5)
plate_c<-c(diff_c_1,diff_c_2,diff_c_3,diff_c_4,diff_c_5)
...
G_plot<-ggplot(tratamientos,aes(Time,value,color=Tratamiento,group=Tratamiento,fill=Tratamiento)) +
  ...
  scale_y_continuous("d(#Generaciones bacterianas)/dt") +
  geom_vline(aes(xintercept = 19), 
             linetype = "dashed", colour = "black",size = 1) +
  geom_vline(aes(xintercept = 37), 
             linetype = "dashed", colour = "black",size = 1) +
