...
a<-c(); b<-c()
for (i in 1:4) {
  a<-complete[,c(24*i-22,24*i-17,24*i-12,24*i-7,44+i)]
  b<-cbind(b,a)
}
...
colnames(data)<-c("Tiempo","0","1.25","2.5","5","10")
...
t_plot<-ggplot(data , aes(Tiempo, value,color=variable, group=variable)) + 
  ...
  scale_color_jcolors(palette = "pal2",name=bquote("AMP ["*mu*"g/ml]")) +
  theme_bw() +
  #new_scale_fill() +
  #geom_tile(data = OD_control,aes(fill=mean)) +
  #scale_fill_viridis_c(option = "turbo",direction = -1,name=bquote(~DO[Control])) +
  #scale_color_distiller(palette = "Dark2",name=bquote("AMP ["*mu~"g/ml]")) +
...
OD_plot<-ggplot(OD , aes(Tiempo, value,color=variable, group=variable,alpha=variable)) + 
  geom_line() + 
  geom_errorbar(aes(ymin=value-std, ymax=value+std), width=.2,alpha=0.5) +
  #facet_grid(~tratamiento,scales="free_x") +
  #scale_fill_gradientn(colours = pal,name="Tratamiento/Control")  +
