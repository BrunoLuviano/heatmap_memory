...
b<-c()
for (i in 1:z) {
  b[i]<-trapz(x[c(52:y),1],x[c(52:y),1+i])
}
...
mean_treats<-mean_treats[,orden]

#agregar el tiempo
mean_treats<-cbind(Time,mean_treats)
stderror_treats<-cbind(Time,stderror_treats)

#agredar tabla de tratamientos
tratamientos<-read.csv(here("14dic2021","data","tabla_tratamientos.csv"))
tratamientos<-tratamientos[,-c(1)]
colnames(tratamientos)<-a
tratamientos<-tratamientos[,orden]
x<-nrow(tratamientos)
x<-c(1:x)
x<-as.character(x)
tratamientos<-cbind(x,tratamientos)
tratamientos<-melt(tratamientos)
#tratamientos$value<-as.character(tratamientos$value)
...
t_plot<-ggplot(mean_treats , aes(time, variable,fill=mean)) + 
  geom_tile() + 
  #facet_grid(~tratamiento,scales="free_x") +
  #scale_fill_gradientn(colours = pal,name="Tratamiento/Control")  +
  scale_fill_distiller(palette = "PuOr",name=bquote(frac(~DO[Tratamiento],~DO[Control]))) +
  theme_classic() +
  new_scale_fill() +
  ...
  scale_y_discrete("Tratamientos",expand=c(0,0)) +
...
tratamientos_plot<-ggplot(tratamientos , aes(x, variable,fill=value)) + 
  geom_tile() + 
  #facet_grid(~tratamiento,scales="free_x") +
  #scale_fill_gradientn(colours = pal,name="Tratamiento/Control")  +
  scale_fill_distiller(palette = "Dark2",name="AMP [ug/mL]") +
  #scale_fill_viridis_c(name="AMP [ug/mL]") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust = 0.5)) +
  scale_x_discrete("Placa",expand=c(0,0)) +
  scale_y_discrete("Tratamientos",expand=c(0,0)) +
  theme(axis.text = element_text(size = 7.5),
        title = element_text(size = 12,face="bold"),
        panel.border= element_rect(size=0.5,color="black",fill=NA)) +
