...
n<-mean_treats[c(21:38),]
o<-mean_treats[c(40:51),]
p<-mean_treats[c(53:58),]
q<-mean_treats[c(60:131),]
...
n<-nrow(mean_treats)
Time<-Time[c(1:n),]
mean_treats<-as.data.frame(mean_treats)
...
colnames(mean_treats)<-c("time","variable","mean")
...
t_plot<-ggplot(mean_treats , aes(time, variable,fill=mean)) + 
  ...
  #scale_fill_viridis_c(option = "turbo",direction = 1,name=bquote(~Generación)) +
  #scale_fill_distiller(palette = "RdBu",name=bquote(~Generaciones)) +
  #scale_fill_manual(values = rainbow(8)) +
  theme_classic() +
  scale_fill_gradientn(colours = c("#1F60FF", "#1F60FF", "#1F9FFF", "#1FBFFF", 
                                   "#00CFFF", "#2AFFFF", "#2AFFFF", "#55FFFF", "#7FFFFF", "#AAFFFF", 
                                   "#FFFF54", "#FFFF54", "#FFF000", "#FFBF00", "#FFA800", "#FF8A00", 
                                   "#FF8A00", "#FF7000", "#FF4D00", "#FF0000"),name=bquote(~Generaciones)) +
  ...
  geom_vline(aes(xintercept = 37.5), 
             linetype = "dashed", colour = "black",size = 1) +
  geom_vline(aes(xintercept = 49.5), 
             linetype = "dashed", colour = "black",size = 1) +
  
  geom_vline(aes(xintercept = 55.5), 
             linetype = "dashed", colour = "black",size = 1) +
  coord_equal()
